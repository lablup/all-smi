...
//! One tick of the local process pass on Linux (issue #428).
//!
...
//!   full tick, as they did when sysinfo's map was walked instead. sysinfo's
//!   map is deliberately not walked: it still holds every tracked PID that
//!   has died since the last full tick, and a walk would put those rows
//!   back.
...
//!   over a one-tick interval (`sampler_linux` explains the defect). A row
//!   whose sample is unreadable takes sysinfo's values.
//!
...
use super::sampler_linux::{ProcessSample, ProcessSampler, Sampled};
...
/// once. sysinfo still recomputes every process's `cpu_usage` from one
/// global interval on this call (see `sampler_linux`), which is why the
/// sampler's values override it.
...
/// and to the sampler's, which is the same `btime + starttime / clk_tck`.
