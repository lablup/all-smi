...
- Uses the native IOReport API (`src/device/macos_native/ioreport.rs`) for energy counters and CPU/GPU residency. The subscription holds only the channels something reads (24 of 383 on an M5 Max, `src/device/macos_native/ioreport/channel_filter.rs`); the one `IOReportCreateSamples` call per sample is still the floor of its cost, because the providers work per sample rather than per channel, so the manager (`src/device/macos_native/manager.rs`, "Collection cadence") samples at most every 1.75 s whatever the poll interval and repeats the previous window in between: at `--interval 1` every second tick samples and each frequency and residency value is a 2 s mean shown twice, while power keeps its publication-timed readings
...
- Reads temperature and system power from the SMC (`src/device/macos_native/smc.rs`) over one connection kept between collections (`SmcSampler`): key info is looked up once per key, keys the SMC lacks included, temperatures are read 2.5 s after the previous read or full read (at `--interval 1` on ticks 0, 3, 5, 8, 10, ..., two reads per 5 s, over the full sensor set), system power and fans every 5 s, both repeated in between, and a connection that fails is reopened on the next collection and read in full
